...
clusterbegin=$($(expr $realclustersize + 5))
recordnum=$(aws route53 list-resource-record-sets --hosted-zone-id Z2ZYS3N4HRA09T | jq .ResourceRecordSets[].Type | wc -l)

ipoverdraft=$(expr $recordnum - $realclustersize)
echo "ipoverdraft = $ipoverdraft"

for (( i = $clusterbegin; i < $recordnum; i++ )); do
  ...
  if [[ $dnsname == "etcd$i.$tldname" ]]; then
    ...
    $etcdip=$(aws route53 list-resource-record-sets --hosted-zone-id $hostedzone | jq .ResourceRecordSets[$i].ResourceRecords[].Value)
    sed -i "s/etcdnum/etcd$i/g" delete.json
